...
1. 读取本文件。
2. 读取 `wiki_memory/当前状态/项目概览.md`、`wiki_memory/当前状态/系统架构.md`、`wiki_memory/当前状态/当前约束.md`、`wiki_memory/当前状态/当前待办.md`。
...
- 工程记忆只在仓库根的 `wiki_memory/` 中维护（包括 `当前状态/`、`决策/`、`知识/`、`日志/`、`模板/` 和 `工具/`），不在插件目录复制。
...
- `wiki_memory/当前状态/`：当前有效项目事实，每个主题只有一个明确 active 版本。
- `wiki_memory/决策/`：架构、技术选型和影响后续工作的工程决策。
- `wiki_memory/知识/`：稳定的模块、流程、规范和运维知识。
- `wiki_memory/日志/`：一次实质任务的追加式历史；不是当前事实的唯一来源。
- `wiki_memory/AGENTS.md`：记忆目录的程序性维护协议；本文件仍负责仓库级边界。

### Front Matter

所有 `wiki_memory/` 下受管长期页面、MOC 和日志必须包含：
...
1. 新建 `wiki_memory/日志/YYYY-MM-DD-任务标题.md`；同名时追加 `-02`、`-03`。
2. 记录目标、已确认决策、检查范围、文件变更、验证摘要、结果、遗留问题和下一步。
3. 只把用户已确认且影响未来工作的结论提升到 `wiki_memory/当前状态/`、`wiki_memory/决策/` 或 `wiki_memory/知识/`；未确认候选只留在 `wiki_memory/日志/` 的“待确认长期记忆”。
4. 新结论替代旧结论时，将旧页面标为 `superseded` 或 `deprecated`，填写 `supersedes` 并保留旧页。
5. 运行 `python wiki_memory/工具/memory_lint.py index`，再运行 `python wiki_memory/工具/memory_lint.py check`。

日志封存后不改写；需更正时新建日志并链接原日志。禁止记录完整聊天、内部推理、隐藏上下文、密钥、令牌或大段输出。

### 固定操作

- `记忆同步`：在 `wiki_memory/日志/` 生成日志、提出长期记忆候选、刷新索引并检查。
- `记忆检索 <关键词>`：先查 `wiki_memory/当前状态/` 和日志 MOC，再按需查知识、决策与历史日志。
- `记忆体检`：运行 `python wiki_memory/工具/memory_lint.py check`，报告缺字段、非法归属、断链、重复 active 主题与孤儿页。
...
1. 完成与任务相称的测试和检查。
2. 执行 `wiki_memory/` 记忆同步，并检查最终 diff 中没有秘密、依赖目录或无关文件。
